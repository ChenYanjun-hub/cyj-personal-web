...
type WorkRole = {
  ...
  deep?: string[];
  participated?: string[];
  items?: { head: string; body?: string; points?: string[] }[];
};
      ...
      items: [
        {
          ...
          points: [
            "调研传统效果图流程在周期、成本与风格一致性上的痛点，提出 AIGC 辅助设计方案并推动落地，成功在 5 个项目引入 AIGC 图像生成与渲染",
            "把效果图生产拆解为可复用业务流程：需求输入 → 空间信息提取 → 提示词生成 → 图像渲染 → 结果筛选 → 人工优化",
            "按空间尺度、建筑风格、功能业态与汇报场景设计差异化提示词与生成策略，用多轮生成、局部重绘、参数调整控制输出",
            "为团队提供工具、提示词与成果优化支持，降低使用门槛，团队渲染效率提升约 48%",
          ],
        },
        {
          head: "作为 AI 产品经理，主导公司内部 AI 产品从 0→1 落地",
          points: [
            "主导【同舟建景规范问答助手】与【合同审查助手】两个垂直场景 RAG 产品全流程：需求定义、PRD 撰写、技术选型、知识库构建、评测体系、红线治理、迭代决策，并以 AI 辅助编码完成原型落地",
            "把「设计师跨 89 部规范翻 PDF」「购销合同初审靠人工」等真实痛点，转化为可演示、可联调、可追溯的 AI 工作台",
            "自建多维评测 + 一票否决机制管控幻觉与编造风险，将编造类红线维度做到工程可控（≥94%）",
            "沉淀完整 PRD 与可迁移的 AI 产品方法论（详见 AI 项目作品集）",
          ],
        },
        {
          head: "总结实践经验，牵头编写单位内部《大模型训练手册》",
          points: [
            "围绕大模型认知、Prompt 方法、业务场景、工具操作与风险规范构建内容体系，把个人实践沉淀为团队可复用的知识产品",
            "持续收集同事使用反馈，迭代操作方法、提示词模板与培训内容，提升团队对生成式 AI 的理解与使用效率",
          ],
        },
      ],
    },
                    ...
                    <div key={i} className="work-item">
                      ...
                      {it.points ? (
                        <ul className="work-item-points">
                          {it.points.map((pt, j) => (
                            <li key={j} className="about-entry-line muted">
                              {pt}
                            </li>
                          ))}
                        </ul>
